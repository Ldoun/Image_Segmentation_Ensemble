    ...
    def train_step(self):
        ...
        for batch in tqdm(self.train_loader, file=sys.stdout): #tqdm output will not be written to logger file(will only written to stdout)
            ...
            if self.dice_loss_ratio < 1.0:
                loss = ((1-self.dice_loss_ratio) * output.loss) + (self.dice_loss_ratio * self.loss_fn(output.logits.softmax(1)[:, 1, :, :], mask))
            else:
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(test_loader, file=sys.stdout):
                ...
                output = self.model(pixel_values=x['pixel_values']).logits.softmax(1)[:, 1, :, :].detach().reshape(x['pixel_values'].shape[0], -1).cpu().numpy().astype(np.float16)
